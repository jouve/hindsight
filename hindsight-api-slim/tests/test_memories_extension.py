...
class InMemoryMemories(MemoriesExtension):
    ...
    async def search(self, *, conn, bank_id, fact_types, query_embedding, query_text, limit, **kwargs):
        ...
        # The engine consumes the three-field result (semantic hits, bm25 hits, graph seeds), so
        # the seam has to hand back that exact shape — a bare tuple would break the caller.
        from hindsight_api.engine.search.retrieval import SemanticBm25Result

        return {ft: SemanticBm25Result(semantic=[], bm25=[], graph_seeds=[]) for ft in fact_types}
